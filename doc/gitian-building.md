...
*Setup instructions for a Gitian build of Fabcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fabcoin
...
- [Building Fabcoin Core](#building-fabcoin-core)
...
Clone the git repositories for fabcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/fabcoinproject/fabcoin --recursive
git clone https://github.com/fabcoinproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/fabcoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the fabcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Fabcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the fabcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fabcoin/.git/
...
./bin/gbuild --commit fabcoin=${COMMIT} --url fabcoin=${URL} ../fabcoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-fabcoin:
./bin/gbuild --commit fabcoin=${COMMIT},cpp-eth-fabcoin=${ETHCOMMIT} --url fabcoin=${URL},cpp-eth-fabcoin=${ETHURL} ../fabcoin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the fabcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fabcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/fabcoinproject/fabcoin-detached-sigs.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fabcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fabcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fabcoin-osx-build.assert
```
...
[fabcoinproject/gitian.sigs](https://github.com/fabcoinproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@fabcoin.org) and he will commit them.