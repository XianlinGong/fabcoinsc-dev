...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/fabcoinproject/fabcoin>";
    const std::string URL_WEBSITE = "<https://fabcoin.org>";
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                fs::path fabcoinStateDir = GetDataDir() / "stateFabcoin";
                bool fStatus = fs::exists(fabcoinStateDir);
                const std::string dirFabcoin(fabcoinStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsFabcoinstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<FabcoinState>(new FabcoinState(dev::u256(0), FabcoinState::openDB(dirFabcoin, hashDB, dev::WithExisting::Trust), dirFabcoin, existsFabcoinstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::fabcoinMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(fabcoinStateDir.string());
                if (fReset) {
