...
/////////////////////////////////////////// fabcoin
#include <fabcoin/fabcoinstate.h>
#include <fabcoin/fabcoinDGP.h>
#include <libethereum/ChainParams.h>
#include <libethashseal/Ethash.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <fabcoin/storageresults.h>


extern std::unique_ptr<FabcoinState> globalState;
...
using ExtractFabcoinTX = std::pair<std::vector<FabcoinTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const int64_t DEFAULT_MAX_TIP_AGE = 30 * 24 * 60 * 60; //fabcoin value is 24 hours. Ours is 30 days in case something causes the chain to get stuck in testnet
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // fabcoin
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashFabcoinRoot,
    const dev::h256& oldHashStateRoot, const std::vector<FabcoinTransaction>& transactions);
...
class FabcoinTxConverter{

public:

    FabcoinTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionFabcoinTransactions(ExtractFabcoinTX& fabcoinTx);
...
private:
    ...
    FabcoinTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<FabcoinTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
