...
FabcoinGUI::FabcoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(_platformStyle)
{
    ...
    fabcoinVersionChecker = new FabcoinVersionChecker(this);

    if(fCheckForUpdates && fabcoinVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(FABCOIN_RELEASES, FABCOIN_RELEASES);
        QString message(tr("New version of Fabcoin wallet is available on the Fabcoin source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void FabcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Fabcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and fabcoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Fabcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Fabcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a fabcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Fabcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void FabcoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool FabcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FabcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FabcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void FabcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FabcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FabcoinGUI::gotoSendTokenPage()
{
...
void FabcoinGUI::gotoReceiveTokenPage()
{
...
void FabcoinGUI::gotoAddTokenPage()
{
...
void FabcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FabcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FabcoinGUI::gotoCreateContractPage()
{
    if (walletFrame) walletFrame->gotoCreateContractPage();
}

void FabcoinGUI::gotoSendToContractPage()
{
    if (walletFrame) walletFrame->gotoSendToContractPage();
}

void FabcoinGUI::gotoCallContractPage()
{
    if (walletFrame) walletFrame->gotoCallContractPage();
}

void FabcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FabcoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void FabcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Fabcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void FabcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void FabcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Fabcoin"); // default title
...
#ifdef ENABLE_WALLET
void FabcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
void FabcoinGUI::incomingTokenTransaction(const QString& date, const QString& amount, const QString& type, const QString& address, const QString& label, const QString& title)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(amount) +
...
bool FabcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FabcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FabcoinGUI::setHDStatus(int hdEnabled)
{
...
void FabcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FabcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FabcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
static bool ThreadSafeMessageBox(FabcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void FabcoinGUI::addDockWindows(Qt::DockWidgetArea area, QWidget* widget)
{
    ...
    menu(0)
{
    ...
    QList<FabcoinUnits::Unit> units = FabcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FabcoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(FabcoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (FabcoinUnits::Unit u : FabcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FabcoinUnits::name(u)), this);
...
{
    setText(FabcoinUnits::name(newUnits));
}
