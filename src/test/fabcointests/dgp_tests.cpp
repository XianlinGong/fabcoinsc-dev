...
#include <fabcointests/test_utils.h>
...
void contractLoading(){
    dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::fabcoinMainNetwork)));
...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, valtype& code1, valtype& code2, valtype& code3, dev::Address addr){
    ...
    std::vector<FabcoinTransaction> txs;
    txs.push_back(createFabcoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createFabcoinTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createFabcoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createFabcoinTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createFabcoinTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createFabcoinTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createFabcoinTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
void checkValue(T value, T value1, T value2, T value3, T value4, size_t i, std::function<bool(T&,T&)> func){
    ...
    if(801 > i && i > 0) // After initializing the tests, the height of the chain 802
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    ...
    dev::eth::EVMSchedule schedule = fabcoinDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    FabcoinDGP fabcoinDGP(globalState.get());
    dev::eth::EVMSchedule schedule = fabcoinDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<FabcoinTransaction> txs;
    txs.push_back(createFabcoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createFabcoinTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createFabcoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_passage_from_0_to_130_three_paramsInstance_test){
    ...
    for(size_t i = 0; i < 1300; i++){
        dev::eth::EVMSchedule schedule = fabcoinDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    contractLoading();
    FabcoinDGP fabcoinDGP(globalState.get());
    uint32_t blockSize = fabcoinDGP.getBlockSize(100);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test2){
    ...
    contractLoading();
    FabcoinDGP fabcoinDGP(globalState.get());
    uint32_t blockSize = fabcoinDGP.getBlockSize(0);
    BOOST_CHECK(blockSize == DEFAULT_BLOCK_SIZE_DGP);
}
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<FabcoinTransaction> txs;
    txs.push_back(createFabcoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createFabcoinTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createFabcoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(block_size_passage_from_0_to_130_three_paramsInstance_test){
    ...
    FabcoinDGP fabcoinDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint32_t blockSize = fabcoinDGP.getBlockSize(i);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    FabcoinDGP fabcoinDGP(globalState.get());
    uint64_t minGasPrice = fabcoinDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    FabcoinDGP fabcoinDGP(globalState.get());
    uint64_t minGasPrice = fabcoinDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<FabcoinTransaction> txs;
    txs.push_back(createFabcoinTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createFabcoinTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createFabcoinTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_passage_from_0_to_130_three_paramsInstance_test){
    ...
    FabcoinDGP fabcoinDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint64_t minGasPrice = fabcoinDGP.getMinGasPrice(i);
        std::function<bool(const uint64_t&, const uint64_t&)> func = compareUint64;
