...
#include "fabcoinstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

FabcoinState::FabcoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = FabcoinState::openDB(_path + "/fabcoinDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

FabcoinState::FabcoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute FabcoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, FabcoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createFabTokenAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            fabcoin::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> FabcoinState::vins() const // temp
{
...
void FabcoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* FabcoinState::vin(dev::Address const& _a) const
{
    return const_cast<FabcoinState*>(this)->vin(_a);
}

Vin* FabcoinState::vin(dev::Address const& _addr)
{
...
// void FabcoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void FabcoinState::kill(dev::Address _addr)
{
...
void FabcoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address FabcoinState::createFabTokenAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void FabcoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void FabcoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void FabcoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
