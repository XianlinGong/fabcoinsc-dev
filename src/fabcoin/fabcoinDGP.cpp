#include "fabcoinDGP.h"

void FabcoinDGP::initDataEIP158(){
...
bool FabcoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule FabcoinDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t FabcoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t FabcoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t FabcoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t FabcoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool FabcoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void FabcoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void FabcoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void FabcoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void FabcoinDGP::createParamsInstance(){
...
dev::Address FabcoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void FabcoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void FabcoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void FabcoinDGP::parseStorageOneUint64(uint64_t& value){
...
void FabcoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule FabcoinDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
