...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 10)
        throw std::runtime_error(
            ...
            + HelpExampleCli("sendtoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"donation\", \"seans outpost\", false, null, null, \"\", \"1X1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\", true")
            + HelpExampleRpc("sendtoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"donation\", \"seans outpost\"")
            + HelpExampleRpc("sendtoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"donation\", \"seans outpost\", false, null, null, \"\", \"1X1GkJdye9WoUnrE2v6ZQhQ72EUVDtGXQX\", true")
        );
...
UniValue createcontract(const JSONRPCRequest& request){
    ...
    uint64_t blockGasLimit = fabcoinDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(fabcoinDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 6)
        throw std::runtime_error(
                ...
                "3. gasPrice  (numeric or string, optional) gasPrice FABCOIN price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
...
UniValue sendtocontract(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
                ...
                "5. gasPrice  (numeric or string, optional) gasPrice Fabcoin price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
