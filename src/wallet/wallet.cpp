...
/**
 * Fees smaller than this (in liu) are considered zero fee (for transaction creation)
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CFabcoinAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/blockchaingate/fabcoin/pull/10340, but that change has been
    // postponed until after https://github.com/blockchaingate/fabcoin/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, CAmount nGasFee, bool hasSender)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-fabcoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CFabcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CFabcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CFabcoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CFabcoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CFabcoinAddress(address).ToString());
}
 ...
 * https://fabcointalk.org/?topic=54527, or
 * https://github.com/blockchaingate/fabcoin/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CFabcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CFabcoinAddress(dest).ToString(), key);
}
