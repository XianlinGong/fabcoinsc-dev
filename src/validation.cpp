...
std::unique_ptr<FabcoinState> globalState;
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = fabcoinDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = fabcoinDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            FabcoinTxConverter converter(tx, NULL);
            ExtractFabcoinTX resultConverter;
            if(!converter.extractionFabcoinTransactions(resultConverter)){
            ...
            std::vector<EthTransactionParams> fabcoinETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(FabcoinTransaction fabcoinTransaction : fabcoinTransactions){
                sumGas += fabcoinTransaction.gas() * fabcoinTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, fabcoinTransaction.gasPrice());
                } else {
                    txMinGasPrice = fabcoinTransaction.gasPrice();
                }
                VersionVM v = fabcoinTransaction.getVersion();
                if(v.format!=0)
                ...
                if(fabcoinTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(fabcoinTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(fabcoinTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += fabcoinTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)fabcoinTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(fabcoinETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > fabcoinTransactions.size())
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s FABCOIN additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // fabcoin
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // fabcoin
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    FabcoinTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<FabcoinTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path fabcoinDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(fabcoinDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(fabcoinDir.string(), std::ios::in | std::ios::out);
...
bool FabcoinTxConverter::extractionFabcoinTransactions(ExtractFabcoinTX& fabcointx){
    std::vector<FabcoinTransaction> resultTX;
    ...
    fabcointx = std::make_pair(resultTX, resultETP);
    return true;
}

bool FabcoinTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool FabcoinTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
FabcoinTransaction FabcoinTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    FabcoinTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = FabcoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = FabcoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    ///////////////////////////////////////////////// // fabcoin
    FabcoinDGP fabcoinDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setFabcoinSchedule(fabcoinDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = fabcoinDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = fabcoinDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = fabcoinDGP.getBlockGasLimit(pindex->nHeight + 1);
...
///////////////////////////////////////////////////////////////////////////////////////// fabcoin
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            FabcoinTxConverter convert(tx, &view, &block.vtx);

            ExtractFabcoinTX resultConvertFabcoinTX;
            if(!convert.extractionFabcoinTransactions(resultConvertFabcoinTX)){
            ...
            if(!CheckMinGasPrice(resultConvertFabcoinTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertFabcoinTX.first, blockGasLimit);
            ...
            for(FabcoinTransaction& qtx : resultConvertFabcoinTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertFabcoinTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertFabcoinTX.first[k].from(), resultConvertFabcoinTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // fabcoin
            globalState->setRootUTXO(oldHashUTXORoot); // fabcoin
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // fabcoin
        uint32_t sizeBlockDGP = fabcoinDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
