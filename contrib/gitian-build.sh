...
VERSION=
commit=false
url=https://github.com/fabcoinproject/fabcoin
ethurl=https://github.com/fabcoinproject/cpp-eth-fabcoin
proc=2
mem=2000
lxc=true
osslTarUrl=http://downloads.sourceforge.net/project/osslsigncode/osslsigncode/osslsigncode-1.7.1.tar.gz
osslPatchUrl=https://fabcoincore.org/cfields/osslsigncode-Backports-to-1.7.1.patch
...
Run this script from the directory containing the fabcoin, gitian-builder, gitian.sigs, and fabcoin-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/fabcoinproject/fabcoin
...
then
	...
	# Make output folder
	mkdir -p ./fabcoin-binaries/${VERSION}
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fabcoin=${COMMIT},cpp-eth-fabcoin=develop --url fabcoin=${url},cpp-eth-fabcoin=${ethurl} ../fabcoin/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/fabcoin-*.tar.gz build/out/src/fabcoin-*.tar.gz ../fabcoin-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fabcoin=${COMMIT},cpp-eth-fabcoin=develop --url fabcoin=${url},cpp-eth-fabcoin=${ethurl} ../fabcoin/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/fabcoin-*.zip build/out/fabcoin-*.exe ../fabcoin-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit fabcoin=${COMMIT},cpp-eth-fabcoin=develop --url fabcoin=${url},cpp-eth-fabcoin=${ethurl} ../fabcoin/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/fabcoin-*.tar.gz build/out/fabcoin-*.dmg ../fabcoin-binaries/${VERSION}
	fi
	popd
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../fabcoin/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/fabcoin-*win64-setup.exe ../fabcoin-binaries/${VERSION}
	    mv build/out/fabcoin-*win32-setup.exe ../fabcoin-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=master ../fabcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/fabcoin-osx-signed.dmg ../fabcoin-binaries/${VERSION}/fabcoin-${VERSION}-osx.dmg
	fi
	popd
